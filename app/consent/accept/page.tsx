import { redirect } from 'next/navigation'
import { createServerSupabaseClient } from '@/lib/supabase/server'
import { CURRENT_DOCUMENT_VERSION } from '@/lib/consent/constants'
import { ConsentForm } from './_components/consent-form'

export const metadata = { title: 'Atualização dos Termos — CAMMES' }

export default async function ConsentAcceptPage() {
  const supabase = await createServerSupabaseClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()

  if (!user) redirect('/login')

  return (
    <main className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="space-y-1 text-center">
          <h1 className="text-2xl font-semibold tracking-tight">CAMMES</h1>
          <p className="text-sm text-muted-foreground">
            Nossos termos foram atualizados (versão {CURRENT_DOCUMENT_VERSION})
          </p>
        </div>
        <p className="text-sm text-muted-foreground">
          Para continuar usando a plataforma, por favor revise e aceite os termos atualizados.
        </p>
        <ConsentForm />
      </div>
    </main>
  )
}
